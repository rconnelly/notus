...
# Build the binary first; smoke scripts call `notus` via NOTUS_BIN or target/debug/notus.
cargo build
PROVIDER=lm_studio FAST_MODEL=gemma4:e4b HEAVY_MODEL=gemma4:e4b NOTUS_BIN=./target/debug/notus bash scripts/smoke_test.sh
...
**Vault structure:** `raw/` (immutable user notes), `wiki/` (published articles), `wiki/.drafts/`, `wiki/sources/`, `wiki/queries/`, `wiki/synthesis/`, `.notus/state.db`

### Query synthesis

- `notus query --save` writes dated Q&A notes to `wiki/queries/`
- `notus query --synthesize` writes published synthesis articles to `wiki/synthesis/`
...
- `config.rs` / `global_config.rs` — Two-tier config: per-vault `notus.toml` + user-level `~/.config/notus/config.toml`
...
- `git_ops.rs` — Auto-commit with `[notus]` prefix, safe undo via `git revert`
...
- **Config loading order:** `--vault` flag → `NOTUS_VAULT` env var → global config default vault → error
- **Git safety:** All auto-operations use `[notus]`-prefixed commits; undo uses `git revert` (never destructive)
